...
URenderer::URenderer(Window& parent) :OGLRenderer(parent) {
	camera = new Camera(0.0f, 0.0f, (Vector3(2987.32f, 2017.49f, 1487.84f)));
	camera1 = new Camera();
...
void URenderer::AutoScene(float dt) {
	...
	if (scene1 && !scene2 && camera->GetYaw() >= 0) {
		if (camPos.x >= 0)
			camPos.x -= 10;
		if (camPos.y <= 500)
			camPos.y += 2;
	...
	if (scene1 && !scene2 && camPos.y >= 500) {
		...
		if (camera->GetPitch() >= -30)
			camera->SetPitch(camera->GetYaw() - 0.5);
		spaceship->SetTransform(spaceship->GetTransform() * Matrix4::Rotation(dt, Vector3(0, 0, 1)));

		camera->SetPosition(camPos);
		if (camPos.z >= 4000 && camPos.y >= 1000 && camera->GetYaw() >= 180) scene2 = true;
	}
...
void URenderer::UpdateScene(float dt) {
	...
	if (Window::GetKeyboard()->KeyTriggered(KEYBOARD_P)) {
		...
		if (autoPlay) {
			...
			camera = new Camera(0.0f, 0.0f, Vector3(tPos.x, tPos.y + 200, tPos.z));
			scene1 = false;
			scene2 = false;
			scene3 = false;
			scene4 = false;
