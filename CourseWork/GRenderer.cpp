...
GRenderer::GRenderer(Window& parent) : OGLRenderer(parent) {
    ...
    light = new Light(Vector3(8216,
        heightmapSize.y * 1.5, 8216), Vector4(1, 1, 1, 1), heightmapSize.z * 10);
...
const float autoplayA = 6;
const float autoplayB = 0.5;
...
void GRenderer::AutoScene(float dt) {
    ...
    if (scene1 && !scene2 && playTime >= 2.0f && playTime <= 4.0f)
    {
        if (camPos.x <= 7245)
            camPos.x += autoplayA;
        if (camPos.y >= 125)
        ...
        if (camPos.x >= 7245 && camPos.y <= 125 && camPos.z >= 5600 && camera->GetYaw() <= 246 && playTime <= 4.0f)
            playTime += dt;
    }

    if (scene1 && !scene2 && playTime >= 4.0f)
    {
        if (camPos.y >= 22)
            camPos.y -= autoplayA / 3;
        camera->SetPosition(camPos);
        ...
        if (camPos.y <= 22 && camera->GetPitch() <= -30) scene2 = true;
    }
    ...
    if (scene1 && scene2 && scene3 && playTime >= 4.0f && playTime <= 5.0f) playTime += dt;
